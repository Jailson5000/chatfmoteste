-- Drop and recreate the view without security_invoker for accurate admin counts
...
SELECT 
  ...
  -- Current usage counts (count ALL profiles, not just active)
  ...
  ), 0) AS current_ai_conversations,
  -- TTS minutes from usage_records
  COALESCE((
    SELECT SUM(ur.duration_seconds) / 60.0 
    FROM usage_records ur 
    WHERE ur.law_firm_id = c.law_firm_id 
      AND ur.usage_type = 'tts_audio'
      AND ur.billing_period = to_char(CURRENT_DATE, 'YYYY-MM')
  ), 0) AS current_tts_minutes
FROM companies c
...
-- Create RLS-like policy via function for the view
COMMENT ON VIEW public.company_usage_summary IS 
'Usage metrics aggregation view for billing purposes. 
Access should be controlled at the application level - only global admins should query this view.';