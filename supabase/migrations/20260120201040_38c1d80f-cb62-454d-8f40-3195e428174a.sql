-- Drop all versions and recreate with correct types
DROP FUNCTION IF EXISTS get_conversations_with_metadata(uuid, integer, integer);

CREATE FUNCTION get_conversations_with_metadata(
  _law_firm_id uuid,
...
RETURNS TABLE (
  ...
  ai_audio_last_enabled_at timestamptz,
  ai_audio_last_disabled_at timestamptz,
  ...
  n8n_last_response_at timestamptz,
