...
-- Criar função auxiliar para testar o agendamento de follow-ups
...
BEGIN
  -- Get current status
  SELECT custom_status_id INTO _old_status_id FROM clients WHERE id = _client_id;
  
  -- Update the client status (this should trigger the normal trigger too)
  UPDATE clients SET custom_status_id = _new_status_id WHERE id = _client_id;
  
  -- Now manually do the logic to debug
  IF _old_status_id IS DISTINCT FROM _new_status_id AND _new_status_id IS NOT NULL THEN
    ...
    IF _conversation IS NOT NULL THEN
      ...
      LOOP
        ...
        INSERT INTO scheduled_follow_ups (
          ...
          _conversation.law_firm_id, _client_id, _conversation.id, _follow_up.id, _follow_up.template_id,
    ...
    ELSE
      _result := jsonb_build_object('success', false, 'error', 'No conversation found');
    END IF;
  ELSE
    _result := jsonb_build_object('success', false, 'error', 'Status did not change or new status is null');
  END IF;
