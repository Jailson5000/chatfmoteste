...
-- Criação segura de agendamentos públicos com validação de tenant
-- =============================================================================
...
BEGIN
  ...
  IF _professional_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Select first available professional linked to the service
    SELECT p.id, p.name
    INTO _professional
    FROM public.agenda_pro_professionals p
    INNER JOIN public.agenda_pro_service_professionals sp ON sp.professional_id = p.id
  ...
  INSERT INTO public.agenda_pro_appointments (
    ...
    source,
    status
    -- confirmation_token uses default gen_random_uuid()
  ) VALUES (
    _law_firm_id,
    _service_id,
    _professional.id,
  ...
  -- 6. Return success with appointment data
  RETURN jsonb_build_object(
    'success', true,
...
-- Grant execute to anon (public booking) and authenticated users
GRANT EXECUTE ON FUNCTION public.create_public_booking_appointment(text, uuid, timestamptz, text, text, uuid, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.create_public_booking_appointment(text, uuid, timestamptz, text, text, uuid, text, text) TO authenticated;

COMMENT ON FUNCTION public.create_public_booking_appointment IS 'Secure RPC for public booking - validates tenant via slug, enforces service/professional rules, and creates appointment with proper isolation';