...
export function AIProviderBadge({ 
  ...
  const { config, isLoading, providerLabel, isN8N, isInternal, isHybrid } = useAIProvider();
  ...
  const getIcon = () => {
    ...
    if (isHybrid) return <Bot className="h-3 w-3" />;
  ...
  const getTooltipContent = () => {
    ...
    if (isHybrid) {
      return "Modo híbrido: Chat usa IA interna, automações usam N8N.";
    }
      ...
      <TooltipTrigger asChild>
        <Badge 
          ...
          className={cn(
            ...
            isHybrid && "bg-purple-500/10 text-purple-600 border-purple-500/20 hover:bg-purple-500/20",
            className
          )}
        >
          {getIcon()}
